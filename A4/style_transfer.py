...
def content_loss(content_weight, content_current, content_original):
    ...
    # C, H, W = content_current.shape
    # content_current = content_current.squeeze().view((C, -1))
    # content_target = content_target.squeeze().view((C, -1))
...
def gram_matrix(features, normalize=True):
    ...
    # print(f"features' shape: {features.permute(0, 2, 1).shape}")
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    # Replace "pass" statement with your code
    style_loss = 0
    for ind, i in enumerate(style_layers):
      feat = feats[i]
      feat_gram = gram_matrix(feat)
      style_loss += style_weights[ind] * \
      ((feat_gram - style_targets[ind]) ** 2).sum()
...
def tv_loss(img, tv_weight):
    ...
    _, _, H, W = img.shape
    # vertical
    rr, cc = torch.meshgrid(torch.arange(H - 1), torch.arange(W))
    loss += ((img[..., rr, cc] - img[..., rr + 1, cc]) ** 2).sum()
    rr, cc = torch.meshgrid(torch.arange(H), torch.arange(W - 1))
    loss += ((img[..., rr, cc + 1] - img[..., rr, cc]) ** 2).sum()
    loss *= tv_weight

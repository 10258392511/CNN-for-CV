...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  conv_w1 = nn.init.kaiming_normal_(torch.empty(channel_1, C, kernel_size_1, 
  kernel_size_1, dtype=dtype, device=device, requires_grad=True))
  conv_b1 = nn.init.zeros_(torch.empty(channel_1, dtype=dtype, device=device))
  conv_w2 = nn.init.kaiming_normal_(torch.empty(channel_2, channel_1, 
  kernel_size_2, kernel_size_2, dtype=dtype, device=device, requires_grad=True))
  conv_b2 = nn.init.zeros_(torch.empty(channel_2, dtype=dtype, device=device, 
  requires_grad=True))
  last_conv_size = H * W * channel_2
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, last_conv_size,  
...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    self.conv1 = nn.Sequential(
      nn.Conv2d(in_channel, channel_1, kernel_size_1, padding=2),
      nn.ReLU()
    )
    self.conv2 = nn.Sequential(
      nn.Conv2d(channel_1, channel_2, kernel_size_2, padding=1),
      nn.ReLU()
    )
    last_conv_size = H * W * channel_2
    # print(f"last: {last_conv_size}")
    self.fc = nn.Linear(last_conv_size, num_classes)
    ############################################################################
    #                           END OF YOUR CODE                            
    ############################################################################
...
def initialize_three_layer_conv_part4():
  ...
  ####################################################################################
  # Replace "pass" statement with your code
  conv_last_size = channel_2 * H * W
  model = nn.Sequential(
    nn.Conv2d(C, channel_1, kernel_size_1, padding=2),
    nn.ReLU(),
    nn.Conv2d(channel_1, channel_2, kernel_size_2, padding=1),
    nn.ReLU(),
    Flatten(),
    nn.Linear(conv_last_size, num_classes)
  )
  optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, 
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential(
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(Cin, Cin, 3, padding=1, stride=stride),
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    for arg in stage_args:
      layers.append(ResNetStage(arg[0], arg[1], arg[2], arg[3], block=block))
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(
      nn.BatchNorm2d(Cin), 
      nn.ReLU(),
      nn.Conv2d(Cin, Cout_shrunk, 1, stride=stride),
      nn.BatchNorm2d(Cout_shrunk),
      nn.ReLU(),
      nn.Conv2d(Cout_shrunk, Cout_shrunk, 3, padding=1),
      nn.BatchNorm2d(Cout_shrunk),
      nn.ReLU(),
      nn.Conv2d(Cout_shrunk, Cout, 1)
    )

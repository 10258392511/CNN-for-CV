...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      h[:, t, :], cache_temp = rnn_step_forward(x[:, t, :], prev_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
      dx[:, t, :], dprev_h_temp, dWx_temp, dWh_temp, db_temp = \
      rnn_step_backward(dprev_h, cache[t])
      dWx += dWx_temp
      dWh += dWh_temp
      db += db_temp
      dprev_h = dprev_h_temp + dh[:, t - 1, :] if t > 0 else dprev_h_temp
      # print(f"{t}: {dprev_h}")
    
    dh0 = dprev_h

    # dh_collect = torch.zeros_like(dh)
    # dh_collect[:, -1, :] = dh[:, -1, :]

    # for t in range(T - 1, -1, -1):
    #   # print(t)
    #   dx[:, t, :], dprev_h_temp, dWx_temp, dWh_temp, db_temp = \
    #   rnn_step_backward(dh_collect[:, t, :], cache[t])
    #   dWx += dWx_temp
    #   dWh += dWh_temp
    #   db += db_temp
    #   dh_collect[:, t - 1, :] = \
    #   dprev_h_temp + dh[:, t - 1, :] if t > 0 else dprev_h_temp
    #   # print(f"{t}: {dh_collec[:, t - 1, :]}")
...
# read through the code as you will write modules on your own later.              #
##############################################################################
class RNN(nn.Module):
  """
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # Replace "pass" statement with your code
    N, T, V = x.shape
    loss = F.cross_entropy(x.reshape((-1, V)), y.reshape((-1,)), 
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # num_last_channels = 1280
        V = len(word_to_idx)
        if_pooling = False if cell_type == "attention" else True
        self.fe = FeatureExtractor(pooling=if_pooling, dtype=dtype, device=device)
        self.cnn2hidden = nn.Linear(input_dim, hidden_dim).to(dtype=dtype, 
        device=device)
        self.embedding = WordEmbedding(V, wordvec_dim, dtype=dtype, 
        device=device)
        self.hidden2prob = nn.Linear(hidden_dim, V).to(dtype=dtype, 
        device=device)
        if cell_type == "rnn":
          self.rnn = RNN(wordvec_dim, hidden_dim, dtype=dtype, device=device)
        elif cell_type == "lstm":
          self.rnn = LSTM(wordvec_dim, hidden_dim, dtype=dtype, device=device)
    ...
    def forward(self, images, captions):
        ...
        ############################################################################
        # Replace "pass" statement with your code
        cnn_features = self.fe.extract_mobilenet_feature(images)
        # print(f"cnn_features' shape: {cnn_features.shape}")
        captions_vec = self.embedding.forward(captions_in)
        if self.cell_type == "rnn" or self.cell_type == "lstm":
          h0 = self.cnn2hidden(cnn_features)
          h = self.rnn.forward(captions_vec, h0) # (N, T, H)
          # scores = self.hidden2prob(h) # (N, T, V)
        ...
        #   pass
          ...
          cnn_features = self.cnn2hidden(cnn_features)
          cnn_features = cnn_features.permute(0, 3, 1, 2)
          # h0 = cnn.features.mean(dim=(2, 3))
          h = self.rnn.forward(captions_vec, cnn_features)
        scores = self.hidden2prob(h) # (N, T, V)
        loss = temporal_softmax_loss(scores, captions_out, 
        ignore_index=self.ignore_index)
        ############################################################################
    ...
    def sample(self, images, max_length=15):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        cur_word = torch.full((N,), self._start)
        cnn_feature = self.fe.extract_mobilenet_feature(images)
        if self.cell_type == "rnn" or self.cell_type == "lstm":
          prev_h = self.cnn2hidden(cnn_feature)
          prev_c = torch.zeros_like(prev_h)
          for l in range(max_length):
            cur_word_vec = self.embedding.forward(cur_word)
            if self.cell_type == "rnn":
              prev_h = self.rnn.step_forward(cur_word_vec, prev_h)
            elif self.cell_type == "lstm":
              prev_h, prev_c = \
              self.rnn.step_forward(cur_word_vec, prev_h, prev_c)
            
            probs = self.hidden2prob(prev_h)
            cur_word = probs.max(dim=-1)[1]
            captions[:, l] = cur_word
        # elif self.cell_type == "lstm":
        #   pass
        elif self.cell_type == "attention":
          cnn_feature = cnn_feature.permute(0, 2, 3, 1)
          cnn_feature = self.cnn2hidden(cnn_feature)
          cnn_feature = cnn_feature.permute(0, 3, 1, 2)
          prev_h = cnn_feature.mean(dim=(2, 3))
          prev_c = prev_h.clone()
          for l in range(max_length):
            cur_word_vec = self.embedding.forward(cur_word)
            attn, attn_w_temp = dot_product_attention(prev_h, cnn_feature)
            attn_weights_all[:, l, :, :] = attn_w_temp
            prev_h, prev_c = self.rnn.step_forward(cur_word_vec, prev_h, 
            prev_c, attn)
            probs = self.hidden2prob(prev_h)
            cur_word = probs.max(dim=-1)[1]
            captions[:, l] = cur_word
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape
    _, H = h0.shape
    prev_h, prev_c = h0.clone(), c0.clone()
    h = torch.zeros((N, T, H), dtype=x.dtype, device=x.device)
    for t in range(T):
...
def dot_product_attention(prev_h, A):
    ...
    weights = prev_h.bmm(A_arr) / math.sqrt(H)  # (N, 1, 16)
    attn_weights = F.softmax(weights, dim=-1)
    attn = (attn_weights * A_arr).sum(dim=-1)
    attn_weights = attn_weights.squeeze().view((N, D_a, D_a))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################

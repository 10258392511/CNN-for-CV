...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      sample_tr = x_train[i].view((-1,))
      sample_te = x_test[j].view((-1,))
      # print(f"{sample_tr.shape}, {sample_te.shape}")
      dists[i, j] = torch.sqrt(((sample_tr - sample_te) ** 2).sum())
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  X_tr = x_train.view((num_train, -1))
  X_te = x_test.view((num_test, -1))

  for j in range(num_test):
    sample_te = X_te[j]
    diffs = (X_tr - sample_te) ** 2
    diffs = diffs.sum(dim=1).sqrt()
    dists[:, j] = diffs
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def compute_distances_no_loops(x_train, x_test):
  ...
  X_tr = x_train.view((num_train, -1))
  X_te = x_test.view((num_test, -1))
  tr_inner = (X_tr ** 2).sum(dim=1)
  te_inner = (X_te ** 2).sum(dim=1)
  # print(f"{tr_inner.shape}, {te_inner.shape}")
  dists = -2 * X_tr @ X_te.t() + tr_inner.unsqueeze(1) + te_inner
  dists = dists.sqrt()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _, inds = torch.topk(-dists, k, dim=0)  # (k, num_test)
  n_classes, _ = y_train.max(dim=0)
  n_classes += 1
  # print(f"{inds}, {inds.shape}")
  inds = inds.view((-1,))
  labels = y_train[inds].view((-1, num_test)).t()
  # print(labels)
  for i in range(num_test):
    hist = torch.histc(labels[i].to(torch.float64), bins=n_classes, min=0, 
    max=n_classes - 1)
    # print(f"{i}: {hist}")
    _, inds_sorted = hist.sort(descending=True)
    y_pred[i] = inds_sorted[0]
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for fold in range(num_folds):
      val_selected_x = x_train_folds[fold]
      val_selected_y = y_train_folds[fold]
      train_selected_x = torch.cat([x_train_folds[i] for i in range(num_folds) 
      if not i == fold], dim=0)
      train_selected_y = torch.cat([y_train_folds[i] for i in range(num_folds) 
      if not i == fold], dim=0)

      dists = compute_distances_no_loops(train_selected_x, val_selected_x)
      y_pred = predict_labels(dists, train_selected_y, k=k)
      acc_doc.append(((y_pred == val_selected_y).sum() / y_pred.shape[0]).item())
    
    k_to_accuracies[k] = acc_doc
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  # keys = k_to_accuracies.keys()
  # num_folds = len(k_to_accuracies[keys[0]])
  num_folds = -1
  for key, val in k_to_accuracies.items():
    num_folds = len(val)
    break
    
  acc_k_avg = {k: sum(accs) / num_folds for k, accs in k_to_accuracies.items()}
  acc_k_avg_sorted = sorted(acc_k_avg.items(), key=lambda P: P[1], reverse=True)
  best_k = acc_k_avg_sorted[0][0]
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k

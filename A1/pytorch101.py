...
def count_tensor_elements(x):
  ...
  # Replace "pass" statement with your code
  size = x.size
  num_elements = torch.prod(torch.tensor(x.shape)).item()
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_elements
...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x = torch.arange((start // 10 + 1) * 10, (stop // 10 + 1) * 10, 10).to(torch.float64)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def make_one_hot(x):
  ...
  # Replace "pass" statement with your code
  N, C = len(x), max(x) + 1
  y = torch.zeros((N, C), dtype=torch.int64)
...
def reshape_practice(x):
  ...
  # Replace "pass" statement with your code
  N = x.shape[0]
  y = torch.cat([x[:N//2].view(3, -1), x[N//2:].view(3, -1)], dim=1)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  y = x.clone()
  ind = y.argmin(dim=1)
  y[torch.arange(y.shape[0]), ind] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def normalize_columns(x):
  ...
  var = ((x - mu) ** 2).sum(dim=0) / (x.size()[0] - 1)
  # print(var)
  std = torch.sqrt(var)
  y = (x - mu) / std
  #############################################################################

...
class Conv(object):
      ...
      for f in range(F):
        for r_out in range(H_out):
          for c_out in range(W_out):
            r_start, c_start = r_out * stride, c_out * stride
            patch = X[n, :, r_start : r_start + HH, c_start : c_start + WW]
            kernel = w[f, :, :, :]
            out[n, f, r_out, c_out] = (patch * kernel).sum() + b[f]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b, conv_param)
  ...
  def backward(dout, cache):
    ...
    # do the complete version with padding first, then crop it
    N, C, H, W = x.shape
    F, _, HH, WW = w.shape
    H_padded = H + 2 * pad
    W_padded = W + 2 * pad
    X = torch.nn.functional.pad(x, (pad, pad, pad, pad), "constant", 0)
    dx_padded = torch.zeros(N, C, H_padded, W_padded, dtype=x.dtype, 
    device=x.device)
      ...
      for f in range(F):
        for r_out in range(H_out):
          for c_out in range(W_out):
            ...
            patch = X[n, :, r_start : r_start + HH, c_start : c_start + WW]
            kernel = w[f, :, :, :]
            dx_padded[n, :, r_start : r_start + HH, c_start : c_start + WW] += \
            dout[n, f, r_out, c_out] * kernel
            dw[f, :, :] += dout[n, f, r_out, c_out] * patch
            db[f] += dout[n, f, r_out, c_out]
    ...
    for n in range(N):
      for c in range(C):
        for r_out in range(H_out):
          for c_out in range(W_out):
            r_start, c_start = r_out * stride, c_out * stride
            patch = x[n, c, r_start : r_start + stride, c_start : c_start + stride]
            out[n, c, r_out, c_out] = patch.max()
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
      ...
      for c in range(C):
        for r_out in range(H_out):
          for c_out in range(W_out):
            ...
            dx[n, c, r_start : r_start + stride, c_start : c_start + stride] *= \
            mask * dout[n, c, r_out, c_out]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    ############################################################################
    # Replace "pass" statement with your code
    self.params["W1"] = torch.randn(num_filters, 3, filter_size, filter_size, 
    dtype=dtype, device=device) * weight_scale
    self.params["b1"] = torch.zeros(num_filters, dtype=dtype, device=device)
    conv_last_side = input_dims[-1] // 2
    conv_last_size = num_filters * (conv_last_side ** 2)
    self.params["W2"] = torch.randn(conv_last_size, hidden_dim, dtype=dtype, 
    device=device) * weight_scale
    self.params["b2"] = torch.zeros(hidden_dim, dtype=dtype, device=device)
    self.params["W3"] = torch.randn(hidden_dim, num_classes, dtype=dtype, 
    device=device) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    N = X.shape[0]
    x = X
    caches = [None for _ in range(4)]
    x, caches[1] = Conv_ReLU_Pool.forward(x, self.params["W1"], 
    self.params["b1"], conv_param, pool_param)
    conv_last_shape = x.shape
    x = x.view((N, -1))
    # print(x.shape)
    x, caches[2] = Linear_ReLU.forward(x, self.params["W2"], self.params["b2"])
    x, caches[3] = Linear.forward(x, self.params["W3"], self.params["b3"])
    scores = x
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dX, grads["W3"], grads["b3"] = Linear.backward(dX, caches[3])
    dX, grads["W2"], grads["b2"] = Linear_ReLU.backward(dX, caches[2])
    dX = dX.view(conv_last_shape)
    _, grads["W1"], grads["b1"] = Conv_ReLU_Pool.backward(dX, caches[1])
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    # Batchnorm scale (gamma) and shift (beta) parameters should be initialized #
    # to ones and zeros respectively.                                          #           
    ############################################################################
    # Replace "pass" statement with your code
    all_filters = [input_dims[0]] + num_filters
    i = -1
    conv_shape_last = input_dims[1]
    for i in range(len(num_filters)):
      ...
      if weight_scale == "kaiming":
        self.params[f"W{i + 1}"] = kaiming_initializer(all_filters[i], 
        all_filters[i + 1], 3, if_relu, dtype=dtype, device=device)
      else:
        self.params[f"W{i + 1}"] = torch.randn(all_filters[i + 1], all_filters[i], 
        3, 3, dtype=dtype, device=device) * weight_scale
      self.params[f"b{i + 1}"] = torch.zeros(all_filters[i + 1], dtype=dtype, 
      device=device)
      if self.batchnorm:
        self.params[f"gamma{i + 1}"] = torch.ones(all_filters[i + 1], 
        dtype=dtype, device=device)
        self.params[f"beta{i + 1}"] = torch.zeros(all_filters[i + 1], 
        dtype=dtype, device=device)
    
    i += 1
    conv_shape_last = (conv_shape_last ** 2) * num_filters[-1]
    if weight_scale == "kaiming":
      self.params[f"W{i + 1}"] = kaiming_initializer(conv_shape_last, 
      num_classes, None, False, dtype=dtype, device=device)
    else:
      self.params[f"W{i + 1}"] = torch.randn(conv_shape_last, num_classes, 
      dtype=dtype, device=device) * weight_scale
    self.params[f"b{i + 1}"] = torch.zeros(num_classes, dtype=dtype, 
    device=device) 
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    caches = [None for _ in range(self.num_layers + 1)]
    pool_caches = [None for _ in range(self.num_layers)]
    x = X
    l = -1
    for l in range(num_filters):
      l += 1
      if self.batchnorm:
        x, caches[l] = Conv_BatchNorm_ReLU.forward(x, self.params[f"W{l}"], 
        self.params[f"b{l}"], self.params[f"gamma{l}"], self.params[f"beta{l}"], 
        conv_param, self.bn_params[l - 1])
      else:
        x, caches[l] = Conv_ReLU.forward(x, self.params[f"W{l}"], 
        self.params[f"b{l}"], conv_param)

      if l - 1 in self.max_pools:
        x, pool_caches[l] = FastMaxPool.forward(x, pool_param)
    
    l = num_filters + 1
    conv_last_shape = x.shape
    x = x.view((N, -1))
    x, caches[l] = Linear.forward(x, self.params[f"W{l}"], self.params[f"b{l}"])
    scores = x
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    # Replace "pass" statement with your code
    loss, dX = softmax_loss(scores, y)
    dX, grads[f"W{l}"], grads[f"b{l}"] = Linear.backward(dX, caches[l])
    dX = dX.view(conv_last_shape)
    l -= 1
    while l > 0:
      if l - 1 in self.max_pools:
        dX = FastMaxPool.backward(dX, pool_caches[l])
      if self.batchnorm:
        dX, grads[f"W{l}"], grads[f"b{l}"], grads[f"gamma{l}"], grads[f"beta{l}"] = \
        Conv_BatchNorm_ReLU.backward(dX, caches[l])
      else:
        dX, grads[f"W{l}"], grads[f"b{l}"] = Conv_ReLU.backward(dX, caches[l])
      l -= 1
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  ################################################################################
  # Replace "pass" statement with your code
  num_filters = [16, 32, 128, 512]
  max_pools = [1, 2, 3]
  model = DeepConvNet(num_filters=num_filters, max_pools=max_pools, reg=0, 
  weight_scale="kaiming", device=device, dtype=dtype)
  optim_config = {"learning_rate": 1e-3 * 3}
  solver = Solver(model, data_dict, update_rule=adam, 
  optim_config=optim_config, lr_decay=0.98, num_epochs=60, batch_size=128, 
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    N, D = x.shape
    # print(f"inside: x's shape: {x.shape}, D: {D}")
    ...
    if mode == 'train':
      ...
      # Replace "pass" statement with your code
      N, D = x.shape
      mean = x.sum(dim=0, keepdim=True) / N
      x_demean = x - mean
      var = (x_demean ** 2).sum(dim=0, keepdim=True) / N
      x_normed = x_demean / (var + eps).sqrt()
      out = x_normed * gamma + beta
      # print(f"mean's shape: {mean.shape}, running mean's: {running_mean.shape}")
      running_mean = momentum * running_mean + (1 - momentum) * mean
      running_var = momentum * running_var + (1 - momentum) * (var + eps)
      cache = (x, mean, var + eps, gamma, beta, mode, running_mean, \
      running_var)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
      ...
      #######################################################################
      # Replace "pass" statement with your code
      out = (x - running_mean) / (running_var + eps).sqrt()
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    else:
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    x, mean, var, gamma, beta, mode, running_mean, running_var = cache
    dx = torch.zeros_like(dout)
    N, D = dout.shape
    if mode == "train":
      x_normed = (x - mean) / var.sqrt()
      dgamma = (dout * x_normed).sum(dim=0)
      dbeta = dout.sum(dim=0)
      dx_normed = dout * gamma
      dx += dx_normed / var.sqrt()

      x_mean = x - mean
      dsigma = -(dx_normed * x_mean).sum(dim=0) / var
      dx += dsigma / var.sqrt() * x / N

      ones_temp = torch.ones(N, dtype=dout.dtype, device=dout.device)
      dmean = -(dx_normed / var.sqrt()).sum(dim=0) - dsigma * mean / var.sqrt()
      # print(f"dmean: {dmean.shape}")
      dx += 1 / N * ones_temp.view((-1, 1)).mm(dmean)
    
    elif mode == "test":
      x_normed = (x - running_mean) / running_var.sqrt()
      dgamma = (dout * x_normed).sum(dim=0)
      dbeta = dout.sum(dim=0)
      dx_normed = dout * gamma
      dx = dx_normed / running_var.sqrt()
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
class SpatialBatchNorm(object):
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = dout.shape
    dX = dout.permute(1, 0, 2, 3).reshape(C, -1).t()
    dX, dgamma, dbeta = BatchNorm.backward(dX, cache)

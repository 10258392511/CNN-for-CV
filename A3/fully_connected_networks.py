...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    self.params["W1"] = torch.randn(input_dim, hidden_dim, dtype=dtype, 
    device=device) * weight_scale
    self.params["b1"] = torch.zeros(hidden_dim, dtype = dtype, device=device)
    self.params["W2"] = torch.randn(hidden_dim, num_classes, dtype=dtype, 
    device=device) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    X1, X1_cache = Linear_ReLU.forward(X, self.params["W1"], self.params["b1"])
    A2, A2_cache = Linear.forward(X1, self.params["W2"], self.params["b2"])
    scores = A2
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    # N = X.shape[0]
    # A2 -= A2.max(dim=1)[0].view((-1, 1))
    # A2 = A2.exp()
    # X2 = A2 / A2.sum(dim=1).view((-1, 1))
    # X2_m_1 = X2.clone()
    # X2_m_1[torch.arange(N), y] -= 1
    # dA2 = X2_m_1
    dX1, grads["W2"], grads["b2"] = Linear.backward(dA2, A2_cache)
    _, grads["W1"], grads["b1"] = Linear_ReLU.backward(dX1, X1_cache)
    for key in self.params.keys():
      if key.find("W") != -1:
        W = self.params[key]
        loss += self.reg * (W * W).sum()
        grads[key] += 2 * self.reg * W 
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for l in range(1, 1 + self.num_layers):
      self.params[f"W{l}"] = torch.randn(all_dims[l - 1], all_dims[l], 
      dtype=dtype, device=device) * weight_scale
      self.params[f"b{l}"] = torch.zeros(all_dims[l], dtype=dtype, device=device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    dropout_caches = [None for _ in range(self.num_layers + 1)] 
    for l in range(1, self.num_layers + 1):
      if l == self.num_layers:
        x, cache = Linear.forward(x, self.params[f"W{l}"], 
        self.params[f"b{l}"])
        caches[l] = cache
        # print(f"last layer: {x.shape}")
        break
        
      x, cache = Linear_ReLU.forward(x, self.params[f"W{l}"], 
      self.params[f"b{l}"])
      caches[l] = cache
      if self.use_dropout:
        x, cache = Dropout.forward(x, self.dropout_param)
        dropout_caches[l] = cache
    ...
    for l in range(self.num_layers, 0, -1):
      ...
      if self.use_dropout:
        dX = Dropout.backward(dX, dropout_caches[l])

      dX, grads[f"W{l}"], grads[f"b{l}"] = Linear_ReLU.backward(dX, caches[l])
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = None
  # Replace "pass" statement with your code
  solver = Solver(model, data_dict, device=device, print_every=1000, 
  num_epochs=60, optim_config={"learning_rate": 1e-2})
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return solver
...
def rmsprop(w, dw, config=None):
  ...
  next_w = w - config["learning_rate"] * dw / (config["cache"].sqrt() \
  + config["epsilon"])
  ###########################################################################
  #                             END OF YOUR CODE                            #
  ###########################################################################
...
def adam(w, dw, config=None):
  ...
  m = config["m"] / (1- config["beta1"] ** t)
  config["v"] = config["beta2"] * config["v"] + (1 - config["beta2"]) * (dw * dw)
  v = config["v"] / (1 - config["beta2"] ** t)
  next_w = w - config["learning_rate"] * m / (v.sqrt() + config["epsilon"])
  #############################################################################
  #                              END OF YOUR CODE                             #
  #############################################################################

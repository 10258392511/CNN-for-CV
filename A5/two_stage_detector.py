...
from single_stage_detector import GenerateAnchor, GenerateProposal, IoU, nms
...
class ProposalModule(nn.Module):
  def __init__(self, in_dim, hidden_dim=256, num_anchors=9, drop_ratio=0.3):
    ...
    self.pred_layer = nn.Sequential(
      nn.Conv2d(in_dim, hidden_dim, 3, 1, padding=1),
      nn.Dropout(p=drop_ratio),
      nn.LeakyReLU(),
      nn.Conv2d(hidden_dim, 6 * num_anchors, 1, 1)
    )
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    preds = self.pred_layer(features)  # (B, 6 * A, H', W')
    preds = preds.view(B, self.num_anchors, -1, H, W)  # (B, 6 * A, H', W') -> (B, A, 6, H', W')
    if mode == "eval":
      conf_scores = torch.sigmoid(preds[:, :, 0 : 2, ...])
      offsets = preds[:, :, 2:, ...]
    elif mode == "train":
      pos_scores_and_anchors = self._extract_anchor_data(preds, 
      pos_anchor_idx)  # (M, 6)
      neg_scores = self._extract_anchor_data(preds[:, :, 0 : 2, ...], 
      neg_anchor_idx)  # (M, 2)
      conf_scores = torch.cat([pos_scores_and_anchors[:, 0 : 2], neg_scores], 
      dim=0)  # (2 * M, 2)
      offsets = pos_scores_and_anchors[:, 2:]
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    pos_anc_inds, neg_anc_inds, _, GT_offsets, GT_classes, pos_anc_coords, _ = \
    ReferenceOnActivatedAnchors(anchors, bboxes, grids, iou_mat, method="FasterRCNN")
    conf_scores, offsets, proposals = self.prop_module(feats, pos_anc_coords, 
    pos_anc_inds, neg_anc_inds)
    conf_loss = ConfScoreRegression(conf_scores, B)
    reg_loss = BboxRegression(offsets, GT_offsets, B)
    total_loss = w_conf * conf_loss + w_reg * reg_loss

    GT_class = GT_classes
    conf_scores = conf_scores[:conf_scores.shape[0] // 2, 0 : 1]
    pos_anchor_idx = pos_anc_inds
    anc_per_img = N_p
    features = feats
    ##############################################################################
    #                               END OF YOUR CODE                             #
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
      ...
      anchors = GenerateAnchor(self.anchor_list.to(grids.device), grids)  # (B, A, H', W', 4)
      # (B, A, 2, H', W'), (B, A, 4, H', W')
      conf_scores, offsets = self.prop_module(feats)
      conf_scores = conf_scores[:, :, 0, ...]  # (B, A, H', W')
      proposals = GenerateProposal(anchors, offsets.permute(0, 1, 3, 4, 2), 
      method="FasterRCNN")  # (B, A, H', W', 4)
      ...
      for batch in range(B):
        ...
        final_conf_probs.append(out_conf_scores)
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    # Replace "pass" statement with your code
    B = images.shape[0]
    # (), (M, 1), (M, 4), (B, C=1280, H', W'), (M,), (M,), ()
    total_loss, conf_scores, proposals, features, GT_class, pos_anchor_idx, anc_per_img = \
    self.rpn(images, bboxes, "all")
    proposals_b = []
    for batch in range(B):
      img_anc_mask = (pos_anchor_idx >= batch * anc_per_img) & \
      (pos_anchor_idx < (batch + 1) * anc_per_img)
      proposals_b.append(proposals[img_anc_mask])
    roi_pooled = torchvision.ops.roi_align(features, proposals_b, 
    (self.roi_output_w, self.roi_output_w), aligned=True).mean(dim=(2, 3))  #(M, C, 2, 2) -> (M, C)
    cls_preds = self.cls_layer(roi_pooled)  # (M, num_cls)
    cls_loss_all = nn.functional.cross_entropy(cls_preds, GT_class, 
    reduction="none")  # (M,)
    cls_loss = 0
    for batch in range(B):
      img_anc_mask = (pos_anchor_idx >= batch * anc_per_img) & \
      (pos_anchor_idx < (batch + 1) * anc_per_img)
      # print(f"{batch}: {cls_loss}, {img_anc_mask}")
      cls_loss += cls_loss_all[img_anc_mask].sum() * 1. / img_anc_mask.sum()
    
    cls_loss /= B
    # print(f"{cls_preds.max(dim=-1)[1]}, {GT_class}")
    total_loss += cls_loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    with torch.no_grad():
      ...
      thresh, nms_thresh, mode="FasterRCNN")
      # batch_size = 1, one image per inference
      batch_inds = [0]
      for proposal in final_proposals:
        batch_inds.append(batch_inds[-1] + proposal.shape[0])
      roi_pooled = torchvision.ops.roi_align(features, final_proposals, 
      (self.roi_output_w, self.roi_output_w), aligned=True).mean(dim=(2, 3))  #(M, C, 2, 2) -> (M, C)
      cls_preds = self.cls_layer(roi_pooled)  # (M, num_cls)
      cls_preds = cls_preds.max(dim=1, keepdim=True)[1]  # (M, 1)
      # print(cls_preds)
      final_class = []
      for i in range(len(batch_inds) - 1):
        cur_ind = batch_inds[i]
        next_ind = batch_inds[i + 1]
        final_class.append(cls_preds[cur_ind : next_ind, :])
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################

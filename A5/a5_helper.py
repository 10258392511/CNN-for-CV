...
def ReferenceOnActivatedAnchors(anchors, bboxes, grid, iou_mat, pos_thresh=0.7, neg_thresh=0.3, method='FasterRCNN'):
  ...
  if method == 'FasterRCNN':
    ...
    # print(f"activated_anc_mask: {activated_anc_mask}")
    activated_anc_mask = activated_anc_mask.max(dim=-1)[0] # Bx(AxH’xW’)
    # print(f"activated_anc_mask, after: {activated_anc_mask}")
    ...
    bboxes = bboxes[:, :4]
    # print(f"activated_anc_ind: {activated_anc_ind}")
    # print(f"activated_anc_ind, after: {activated_anc_ind / float(activated_anc_mask.shape[-1])}")
    activated_anc_ind = (activated_anc_ind / float(activated_anc_mask.shape[-1])).long()
    # print(f"activated_anc_ind, long: {activated_anc_ind}")

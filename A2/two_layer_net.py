...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    scores_reg = scores - row_max.view((-1, 1))
    scores_exp = torch.exp(scores_reg)
    scores_normed = scores_exp / scores_exp.sum(dim=1).view((-1, 1))
    loss = -1 / N * scores_normed[torch.arange(N), y].log().sum() \
    + reg * (torch.sum(W1 * W1) + torch.sum(W2 * W2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    # Replace "pass" statement with your code
    da2 = scores_normed
    da2[torch.arange(N), y] -= 1
    da2 /= N
    # one_N = torch.ones_like(scores[:, 0])
    grads["W2"] = h1.t().mm(da2) + 2 * reg * W2
    grads["b2"] = da2.sum(dim=0)
    dh1 = da2.mm(W2.t())
    da1 = dh1.clone()
    da1[h1 == 0] = 0
    grads["W1"] = X.t().mm(da1) + 2 * reg * W1
    grads["b1"] = da1.sum(dim=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
  ...
  regularization_strengths = [0, 1e-5, 1e-4]
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  for lr in lrs:
    for hs in hss:
      for reg in rss:
        for learning_rate_decay in lrds:
          key = (lr, hs, reg, learning_rate_decay)
          ...
          cand_val_acc = stats["val_acc_history"][-1]
          print(f"training with lr {lr}, hs {hs}, rs {reg}, lrd {learning_rate_decay}, "
          , end="")
          print(f"cur acc: {cand_val_acc}, best acc: {best_val_acc}")

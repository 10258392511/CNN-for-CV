...
def svm_loss_vectorized(W, X, y, reg):
  ...
  N, _ = X.shape
  scores = X.mm(W)
  mask = torch.full_like(scores, True, dtype=bool)
  mask[torch.arange(0, N), y] = False
  label_scores = scores[torch.arange(0, N), y]
  scores = scores - label_scores.view((-1, 1)) + delta
  mask_gt_0 = (scores > 0)
  mask_all = mask * mask_gt_0
  loss = (scores * mask_all).sum().item() / N + reg * torch.sum(W * W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  ...
  #############################################################################
  # Replace "pass" statement with your code
  dscores_masked = torch.ones_like(scores) * mask_all
  mask_count = mask_all.sum(dim=1)
  dscores_masked[torch.arange(0, N), y] -= mask_count
  dW = dW + X.t().mm(dscores_masked) / N + 2 * reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def sample_batch(X, y, num_train, batch_size):
  ...
  if num_train >= batch_size:
    inds = random.sample(range(num_train), batch_size)
  else:
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-3, 1e-2, 0.5 * 1e-2, 1e-1]
  regularization_strengths = [0.5, 0.1, 0.05, 0.01]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  # Replace "pass" statement with your code
  num_tr, num_val = data_dict["X_train"].shape[0], data_dict["X_val"].shape[0]
  train_loss = cls.train(data_dict["X_train"], data_dict["y_train"], lr, reg, 
  num_iters)
  preds_tr = cls.predict(data_dict["X_train"])
  preds_val = cls.predict(data_dict["X_val"])
  train_acc = (data_dict["y_train"] == preds_tr).sum().item() / num_tr
  val_acc = (data_dict["y_val"] == preds_val).sum().item() / num_val
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(N):
    x = X[i, :]
    pred = W.t().mv(x)  # (C,)
    max_pred, _ = pred.max(dim=0)
    pred -= max_pred
    pred_exp = torch.exp(pred)
    pred_exp /= pred_exp.sum()
    loss += -torch.log(pred_exp[y[i]])
    weights_deriv = pred_exp
    weights_deriv[y[i]] -= 1
    dW += x.view((-1, 1)).mm(weights_deriv.view((1, -1)))
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  N, _ = X.shape
  lin_scores = X.mm(W)
  max_scores, _ = lin_scores.max(dim=1)
  lin_scores -= max_scores.view((-1, 1))
  exp_scores = torch.exp(lin_scores)
  distribution = exp_scores / exp_scores.sum(dim=1).view((-1, 1))
  loss -= torch.log(distribution[torch.arange(N), y]).sum().item() / N
  loss += reg * torch.sum(W * W)

  weights_deriv = distribution
  weights_deriv[torch.arange(N), y] -= 1
  dW += X.t().mm(weights_deriv)
  dW = dW / N + 2 * reg * W
  #############################################################################
  #                          END OF YOUR CODE                                 #
  #############################################################################
